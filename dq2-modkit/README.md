...
先进入项目目录。最推荐把 `dq2-modkit` 放在游戏根目录下，也就是和 `Game.exe`、`www` 同级；这种布局不需要配置游戏路径，直接启动即可：
...
只有当 `dq2-modkit` 不在游戏根目录下时，才需要复制本地配置并把 `gameRoot` 改成自己的游戏根目录：
...
推荐把这个目录放在游戏根目录下，此时工具会自动用 `dq2-modkit` 的父目录作为游戏目录。也可以放在任意位置，只要通过 `-GameRoot` 或 `config.local.json` 指定游戏根目录即可；`DQ2_GAME_ROOT` 仍支持，但优先级低于项目父目录，避免旧环境变量把 GUI 带到别的游戏目录。
...
当前随项目保存的位置是 `dq2-modkit/skills`。如果要安装到 Codex 的全局 skill 目录用于自动发现，目录名建议保持为 `dq2-modkit-builder`。
