(function () {
    if (window.__codexLocalTrainerBridge)
        return;
    const bridge = {
        version: "0.2.29",
    ...
    const projectRoot = process.env.DQ2_MODKIT_ROOT || path.join(gameRoot, "dq2-modkit");
    ...
    function uniqueTargets(targets) {
        ...
        return targets.filter((target) => {
            if (!target || !target.object || seen.includes(target.object))
                return false;
    ...
    function patchStorageObject(storage, label) {
        if (!storage || storage.__codexSavePathPatched)
            return false;
        try {
            const original = {
    ...
    function refreshTitleContinueCommand() {
        try {
            ...
            if (commandWindow && typeof commandWindow.refresh === "function") {
                ...
                if (typeof commandWindow.activate === "function")
    ...
    function safeGold(party) {
        if (!party)
            return null;
        try {
            if (typeof party.gold === "function")
                return party.gold();
            if (typeof party._gold === "number")
                return party._gold;
        }
        catch (_) { }
        return null;
    ...
    function bumpFishingStat(name, payload) {
        if (bridge.suppressFishingStats > 0)
            return;
    ...
    function isInBattleRewardContext() {
        if (bridge.rateDepth > 0)
            return true;
        try {
            const party = resolveParty();
            if (party && typeof party.inBattle === "function" && party.inBattle())
                return true;
        }
        catch (_) { }
        try {
            ...
            if (battle && battle._phase && battle._phase !== "init")
                return true;
        }
        catch (_) { }
        return false;
    ...
    function isActorBattler(battler) {
        try {
            if (!battler)
                return false;
            if (typeof battler.isActor === "function")
    ...
    function setBattlerHp(battler, value) {
        ...
        withInvincibleSuppressed(() => {
            if (typeof battler.setHp === "function")
                battler.setHp(value);
            else
                battler._hp = value;
        });
    ...
    function shouldBlockHpDecrease(battler, value) {
        if (!bridge.options.invincible || bridge.suppressInvincible > 0)
            return false;
    ...
    function restoreInvincibleHp(battler, snapshot, source) {
        if (!bridge.options.invincible || !isActorBattler(battler) || !Number.isFinite(snapshot))
            return false;
    ...
    function getPartyMembers(party) {
        if (!party)
            return [];
        try {
            if (typeof party.allMembers === "function")
                return party.allMembers().filter(Boolean);
            if (typeof party.members === "function")
    ...
    function currentMapInfo() {
        ...
        let through = null;
        try {
            if (map && typeof map.mapId === "function")
                mapId = map.mapId();
            else if (map && map._mapId != null)
                mapId = map._mapId;
        }
        catch (_) { }
        try {
    ...
    function isInBattle() {
        ...
        try {
            ...
            if (manager && manager._phase && manager._phase !== "init")
                return true;
        }
        catch (_) { }
        return false;
    ...
    function troopEnemies(aliveOnly) {
        ...
        if (!troop)
            return [];
        try {
            if (aliveOnly && typeof troop.aliveMembers === "function")
                return troop.aliveMembers().filter(isEnemyBattler);
            if (typeof troop.members === "function")
                return troop.members().filter(isEnemyBattler);
        }
        catch (_) { }
        if (Array.isArray(troop._enemies))
            return troop._enemies.filter(enemy => isEnemyBattler(enemy) && (!aliveOnly || battlerHp(enemy) > 0));
        return [];
    }
    function defeatEnemy(enemy, source) {
        if (!enemy || !isEnemyBattler(enemy) || battlerHp(enemy) <= 0)
            return false;
        try {
            ...
            if (typeof enemy.die === "function")
                enemy.die();
            if (typeof enemy.performCollapse === "function")
                enemy.performCollapse();
            if (enemy.result && typeof enemy.result === "function") {
                const result = enemy.result();
                if (result)
                    result.hpDamage = Math.max(Number(result.hpDamage || 0), 999999);
            }
            if (typeof enemy.refresh === "function")
                enemy.refresh();
    ...
    function killBattleEnemies(command) {
        ...
        if (count > 0 && finish) {
            try {
                const manager = battleManagerObject();
                if (manager && typeof manager.processVictory === "function")
    ...
    function escapeBattle() {
        ...
        if (!manager || !isInBattle())
            return { attempted: false, escaped: false, reason: "not in battle" };
        let escaped = false;
        try {
            if (typeof manager.processEscape === "function") {
                ...
                if (previousRatio != null)
                    manager._escapeRatio = previousRatio;
            }
    ...
    function playerThroughState(player) {
        try {
            if (player && typeof player.isThrough === "function")
                return !!player.isThrough();
            if (player && player._through != null)
                return !!player._through;
        }
        catch (_) { }
        return false;
    }
    function setPlayerThrough(command, toggle) {
        const player = resolvePlayer();
        if (!player)
            throw new Error("game player is unavailable");
        const previous = playerThroughState(player);
        const next = toggle ? !previous : (Object.prototype.hasOwnProperty.call(command || {}, "value") ? toBool(command.value) : true);
        if (typeof player.setThrough === "function")
            player.setThrough(next);
        else
    ...
    function commandBattleTroopId(command) {
        ...
        if (Number.isFinite(direct) && direct > 0)
            return { troopId: direct, source: "command" };
    ...
    function runBattleInterpreterCommand(troopId, canEscape, canLose) {
        ...
        if (!interpreter || typeof interpreter.command301 !== "function")
            return null;
        const previousParams = interpreter._params;
        const previousIndent = interpreter._indent;
        try {
            if (!interpreter._branch || typeof interpreter._branch !== "object")
                interpreter._branch = {};
    ...
    function runDirectBattleStart(troopId, canEscape, canLose) {
        ...
        if (!manager || typeof manager.setup !== "function")
            throw new Error("BattleManager.setup is unavailable");
        const sceneManager = resolveSceneManager();
        ...
        if (typeof manager.setEventCallback === "function")
            manager.setEventCallback(function () { });
        try {
            const player = resolvePlayer();
            if (player && typeof player.makeEncounterCount === "function")
    ...
    function startSpecifiedBattle(command) {
        if (isInBattle())
            return { started: false, reason: "already in battle", inBattle: true };
        ...
        if (!troop)
            throw new Error(`troop ${troopId} not found`);
    ...
    function requireActor(actorId) {
        ...
        if (!actor)
            throw new Error(`actor ${actorId} is unavailable`);
        return actor;
    }
    function refreshActor(actor) {
        try {
            if (actor && typeof actor.refresh === "function")
                actor.refresh();
        }
        catch (_) { }
    }
    ...
    function equipmentKind(item) {
        if (!item || typeof item !== "object")
            return "";
        if (item.wtypeId != null)
            return "weapon";
        if (item.atypeId != null)
            return "armor";
        const id = Number(item.id || 0);
        for (const kind of ["weapon", "armor"]) {
            const table = resolveData(kind);
            if (Array.isArray(table) && table[id] === item)
                return kind;
        }
        return "";
    }
    ...
    function refreshMapAndWindows() {
        try {
            ...
            const sceneManager = resolveSceneManager();
            const scene = sceneManager && sceneManager._scene;
            if (scene && scene._statusWindow && typeof scene._statusWindow.refresh === "function")
                scene._statusWindow.refresh();
            if (scene && scene._itemWindow && typeof scene._itemWindow.refresh === "function")
                scene._itemWindow.refresh();
            if (scene && scene._skillWindow && typeof scene._skillWindow.refresh === "function")
    ...
    function setTrainerOptions(options) {
        ...
        if (Object.prototype.hasOwnProperty.call(options, "expRate"))
            bridge.options.expRate = clampNumber(options.expRate, 0, 999, bridge.options.expRate);
        if (Object.prototype.hasOwnProperty.call(options, "goldRate"))
            bridge.options.goldRate = clampNumber(options.goldRate, 0, 999, bridge.options.goldRate);
        if (Object.prototype.hasOwnProperty.call(options, "dropRate"))
            bridge.options.dropRate = clampNumber(options.dropRate, 0, 999, bridge.options.dropRate);
        if (Object.prototype.hasOwnProperty.call(options, "skillRate"))
            bridge.options.skillRate = clampNumber(options.skillRate, 0, 999, bridge.options.skillRate);
        if (Object.prototype.hasOwnProperty.call(options, "noSkillCost"))
            bridge.options.noSkillCost = toBool(options.noSkillCost);
        if (Object.prototype.hasOwnProperty.call(options, "oneHitKill"))
            bridge.options.oneHitKill = toBool(options.oneHitKill);
        if (Object.prototype.hasOwnProperty.call(options, "invincible"))
            bridge.options.invincible = toBool(options.invincible);
        if (previousNoCost !== bridge.options.noSkillCost) {
            resetNoCostBaselines();
            if (bridge.options.noSkillCost)
                preserveNoCostResources("enabled");
        }
    ...
    function setFishingOptions(options) {
        ...
        if (Object.prototype.hasOwnProperty.call(options, "autoSuccess"))
            bridge.fishingOptions.autoSuccess = toBool(options.autoSuccess);
        if (Object.prototype.hasOwnProperty.call(options, "powerRate"))
            bridge.fishingOptions.powerRate = clampNumber(options.powerRate, 0, 999, bridge.fishingOptions.powerRate);
        if (Object.prototype.hasOwnProperty.call(options, "powerBonus"))
            bridge.fishingOptions.powerBonus = clampNumber(options.powerBonus, -999999, 999999, bridge.fishingOptions.powerBonus);
        patchTrainerHooks();
        return { ...bridge.fishingOptions };
    }
    function patchFishingGauge(gauge, source) {
        if (!gauge || typeof gauge !== "object" || gauge.__codexFishingGaugePatched)
            return gauge;
    ...
    function patchMethod(owner, name, key, wrapper) {
        if (!owner || typeof owner[name] !== "function")
            return false;
        if (owner[name].__codexTrainerPatched)
            return true;
        if (!bridge.originals[key])
            bridge.originals[key] = owner[name];
    ...
    function patchTrainerHooks() {
        ...
        uniqueTargets(resolvePrototypeTargets("Game_Party", ["Game_Party", "GameParty"]).concat([
            ...
            if (patchMethod(target.object, "fishPower", `${target.label}.fishPower`, function (original, args) {
                ...
                if ((!Number.isFinite(base)) || (rate === 1 && bonus === 0))
                    return base;
        ...
        const enemyProtos = uniqueTargets(resolvePrototypeTargets("Game_Enemy", ["Game_Enemy", "GameEnemy"]).concat(troopEnemyPrototypeTargets("runtime.troop")));
        enemyProtos.forEach((target) => {
            ...
            if (patchMethod(target.object, "makeDropItems", `${target.label}.makeDropItems`, function (original, args) {
                const result = original.apply(this, args);
                if (!Array.isArray(result) || bridge.options.dropRate <= 1)
                    return result;
        ...
        uniqueTargets(resolvePrototypeTargets("Game_Actor", ["Game_Actor", "GameActor"]).concat(partyMemberPrototypeTargets("runtime.party"))).forEach((target) => {
            if (patchMethod(target.object, "gainExp", `${target.label}.gainExp`, function (original, args) {
        ...
        resolvePrototypeTargets("Game_Action", ["Game_Action", "GameAction"]).forEach((target) => {
            if (patchMethod(target.object, "apply", `${target.label}.apply`, function (original, args) {
                ...
                if (hpSnapshot != null)
                    restoreInvincibleHp(targetBattler, hpSnapshot, `${target.label}.apply`);
        ...
        uniqueTargets(resolvePrototypeTargets("Game_Battler", ["Game_Battler", "GameBattler"]).concat(partyMemberPrototypeTargets("runtime.party"))).forEach((target) => {
        ...
        uniqueTargets(resolvePrototypeTargets("Game_BattlerBase", ["Game_BattlerBase", "GameBattlerBase"]).concat(partyMemberPrototypeTargets("runtime.party"))).forEach((target) => {
            if (patchMethod(target.object, "setHp", `${target.label}.setHp`, function (original, args) {
                if (shouldBlockHpDecrease(this, args[0])) {
                    const current = battlerHp(this);
    ...
    function setVariableValue(id, value) {
        const variables = resolveVariables();
        if (!variables || typeof variables.setValue !== "function")
            throw new Error("game variables are unavailable");
    ...
    function callHangupMethod(methodName, statName) {
        const party = resolveParty();
        if (!party || typeof party[methodName] !== "function")
    ...
    function normalizeDropKind(kind) {
        ...
        if (value === "1" || value === "item")
            return "item";
        if (value === "2" || value === "weapon")
            return "weapon";
        if (value === "3" || value === "armor" || value === "armour")
            return "armor";
        return "";
    }
    ...
    function dropTable(kind) {
        ...
        if (kind === "weapon")
            return dataTable("weapon");
        if (kind === "armor")
            return dataTable("armor");
        return [];
    }
    function itemKindOfObject(item) {
        ...
        for (const kind of ["item", "weapon", "armor"]) {
            ...
            if (table && table[id] === item)
                return kind;
        }
        if (item.wtypeId != null || item._wtypeId != null)
            return "weapon";
        if (item.atypeId != null || item._atypeId != null)
            return "armor";
        if (item.itypeId != null || item._itypeId != null || item.consumable != null)
            return "item";
        for (const kind of ["item", "weapon", "armor"]) {
            const table = dataTable(kind);
            if (table && table[id] && table[id].name === item.name)
                return kind;
        }
    ...
    function addDropGroup(groups, item, count) {
        ...
        if (!groups[key])
            groups[key] = { ...summary, count: 0, _items: [] };
        groups[key].count += count || 1;
        if (groups[key]._items.length < 20)
    ...
    function itemQuality(item, kind) {
        ...
        if (tableKind === "weapon" || tableKind === "armor") {
            ...
            if (base && base !== item)
                return itemQuality(base, tableKind);
        }
        return null;
    }
    ...
    function itemContainerForKind(party, kind) {
        if (!party)
            return null;
        if (kind === "weapon")
            return party._weapons || null;
        if (kind === "armor")
            return party._armors || null;
        if (kind === "item")
            return party._items || null;
        return null;
    }
    ...
    function gainOfflineItem(party, item) {
        if (!party || typeof party.gainItem !== "function" || !item)
    ...
    function noteEnemyDrops(enemy, rate) {
        ...
        match[1].split(/\r?\n/).forEach((line) => {
            ...
            if (!item || !Number.isFinite(percent))
                return;
        ...
        preview.enemies.forEach((enemyRow) => {
            ...
            (enemy && enemy.dropItems || []).forEach((drop) => {
                if (!drop || !drop.kind || !drop.dataId)
                    return;
                const kind = normalizeDropKind(drop.kind);
                if (!kind || noteDropKeys.has(`${kind}:${drop.dataId}`))
                    return;
    ...
    function offlineHuntProbe(command) {
        ...
        if (!encounters.length)
            throw new Error(`map ${mapId} has no encounter list`);
        ...
        for (let index = 0; index < times; index += 1) {
            ...
            const preview = troopDataPreview(troopId);
            if (!troops[troopId])
                troops[troopId] = { id: troopId, name: preview && preview.name || "", count: 0 };
    ...
    function offlineEncounterList(mapId, regionId) {
        ...
        if (!data || !Array.isArray(data.encounterList))
            return [];
    ...
    function offlineHuntMapPreview(command) {
        ...
        if (!map)
            throw new Error(`map ${mapId} data is unavailable`);
    ...
    function offlineHuntTroopPreview(command) {
        ...
        const preview = troopDataPreview(troopId);
        if (!preview)
            throw new Error(`troop ${troopId} data is unavailable`);
        const rateExp = Number(bridge.options.expRate || 1);
        const rateGold = Number(bridge.options.goldRate || 1);
        return {
            mode: "troop",
    ...
    function offlineHuntPreview(command) {
        if (command && command.troopId != null && command.troopId !== "")
            return offlineHuntTroopPreview(command);
    ...
    function revealEnemyBookIds(enemyIds) {
        ...
        if (!Array.isArray(config.enemyBook))
            config.enemyBook = [];
        ids.forEach((id) => {
    ...
    function saveGameToSlot(savefileId) {
        ...
        if (!dataManager || typeof dataManager.saveGame !== "function")
            throw new Error("saveGame is unavailable");
        const id = Math.floor(requireNumber(savefileId || 1, "id"));
        if (id <= 0)
            throw new Error("save slot id must be positive");
    ...
    function recoverPartyMembers() {
        ...
        members.forEach(actor => {
            if (actor && typeof actor.recoverAll === "function")
                actor.recoverAll();
            else {
                if (actor && typeof actor.setHp === "function" && actor.mhp != null)
                    actor.setHp(actor.mhp);
                if (actor && typeof actor.setMp === "function" && actor.mmp != null)
                    actor.setMp(actor.mmp);
                if (actor && typeof actor.setTp === "function")
                    actor.setTp(100);
            }
            refreshActor(actor);
        });
    ...
    function runOfflineHunt(command) {
        const party = resolveParty();
        if (!party)
            throw new Error("game party is unavailable");
        const fixedTroopId = command.troopId == null || command.troopId === "" ? 0 : Math.floor(requireNumber(command.troopId, "troopId"));
        const mapId = fixedTroopId > 0 && (command.mapId == null || command.mapId === "")
            ? 0
            : Math.floor(requireNumber(command.mapId, "mapId"));
        ...
        for (let index = 0; index < times; index += 1) {
            ...
            if (!reward)
                continue;
            if (String(reward.source || "").startsWith("runtime"))
                runtimeCount += 1;
            else
                dataCount += 1;
            ...
            if (!troopGroups[troopKey])
                troopGroups[troopKey] = { id: troopId, name: preview && preview.name || "", count: 0 };
            troopGroups[troopKey].count += 1;
            (preview && preview.enemies || []).forEach((enemy) => {
                if (!enemyGroups[enemy.id])
                    enemyGroups[enemy.id] = { id: enemy.id, name: enemy.name || "", count: 0 };
        ...
        withRatesSuppressed(() => {
            ...
            if (typeof party.gainGold === "function")
                ...
                party._gold = Math.max(0, Number(party._gold || 0) + gold);
            keptItems.forEach((item) => {
                if (lootConfig.nativeDrops) {
                    ...
                    if (typeof party.gainItem === "function")
                        party.gainItem(item, 1);
                    addDropGroup(dropGroups, item, 1);
                }
            });
        ...
        let recovered = null;
        if (toBool(command.recover))
            recovered = recoverPartyMembers();
        let saved = null;
        if (toBool(command.save))
            saved = saveGameToSlot(command.saveSlot || 1);
        const result = {
            mode: fixedTroopId > 0 ? "troop" : "map",
            mapId,
            times,
    ...
    function collectState() {
        ...
        return {
            ...
            storagePatched: !!(window.StorageManager && window.StorageManager.__codexSavePathPatched ||
    ...
    function commandIds(command) {
        ...
        if (command.id != null && command.id !== "")
            return uniqueNumericIds([command.id]);
        return [];
    }
    function requireConfigManager() {
        const config = resolveConfigManager();
        if (!config)
            throw new Error("ConfigManager is unavailable");
        return config;
    }
    function saveConfig() {
        const config = requireConfigManager();
    ...
    function sanitizeEnemyWeaknessStore() {
        ...
        if (!system)
            return { available: false, repaired: 0, initialized: false };
        ...
        if (repaired > 0)
            bridge.enemyWeaknessRepair = { ts: Date.now(), repaired };
    ...
    function unlockEnemyBook(ids) {
        ...
        if (!targetIds.length)
            throw new Error("enemy ids are unavailable");
        if (!Array.isArray(config.enemyBook))
            config.enemyBook = [];
    ...
    function commandQueueId(command, line) {
        if (!command || typeof command !== "object")
            return "";
        if (command.__codexQueueId)
            return String(command.__codexQueueId);
        if (command.commandId || command._commandId || command.cid) {
            return String(command.commandId || command._commandId || command.cid);
        }
        if (typeof command.id === "string" && /^\d+-[a-f0-9]+$/i.test(command.id))
            return command.id;
    ...
    function readRuntimePath(pathText) {
        ...
        if (!raw || raw === "window")
            return { path: "window", value: window };
        ...
        if (aliasCall)
            return { path: raw, value: applyRuntimePathSuffix(callAlias(aliasCall[1]), aliasCall[2]) };
        ...
        for (let index = start; index < parts.length; index += 1) {
            if (value == null)
                return { path: raw, value: undefined };
            value = value[parts[index]];
        }
        ...
        if (command.prototype !== false && value && (typeof value === "object" || typeof value === "function")) {
            ...
            while (proto && proto !== Object.prototype && rows.length < maxKeys && depth < 3) {
                safeOwnPropertyNames(proto).sort().forEach(key => {
                    if (key !== "constructor")
                        pushKey(proto, key, `proto${depth + 1}`);
                });
    ...
    function runtimeSearch(command) {
        ...
        const shouldDescend = (pathText, key, value, depth) => {
            ...
            if (visited.includes(value))
                return false;
            if (/^(document|localStorage|sessionStorage|indexedDB|chrome|nw|process|require|module|exports|global|console|performance)$/i.test(String(key)))
                return false;
            if (matches(pathText) || matches(key))
                return true;
        ...
        const addResult = (pathText, key, value, source) => {
            ...
            if (!matches(pathText) && !matches(key) && !matches(preview))
                return;
            results.push({
                path: pathText,
                key,
        ...
        const visit = (pathText, object, depth) => {
            if (!object || (typeof object !== "object" && typeof object !== "function"))
                return;
            if (visited.includes(object) || results.length >= maxResults)
                return;
            visited.push(object);
            for (const key of safeOwnPropertyNames(object).sort()) {
                ...
                if (shouldDescend(childPath, key, value, depth))
                    visit(childPath, value, depth + 1);
            }
    ...
    function fishingItemRole(item) {
        ...
        const text = `${name}\n${description}\n${note}`;
        if (/<\s*钓竿\s*:/i.test(note) || /鱼竿/.test(description))
            return "rod";
        if (/<\s*鱼饵\s*:/i.test(note) || /鱼饵/.test(description))
            return "bait";
        if (!/(鱼|蟹|鱿)/.test(name))
            return "";
        if (/鱼竿|鱼饵|食谱|设计图|资格证|奖章|卷轴|定位仪/.test(name + description))
            return "";
        if (/食材|珍宝鱼|材料/.test(description))
            return "fish";
        return "";
    }
    function addFishingItems(command) {
        ...
        if (!party || typeof party.gainItem !== "function")
            throw new Error("party gainItem is unavailable");
        const items = resolveData("item");
        if (!items)
            throw new Error("item data is unavailable");
        const amount = Math.max(1, Math.floor(requireNumber(command.amount || 1, "amount")));
        const roles = new Set();
        if (Array.isArray(command.roles))
            command.roles.forEach(role => roles.add(String(role)));
        if (command.rods || command.rod)
            roles.add("rod");
        if (command.baits || command.bait)
            roles.add("bait");
        if (command.fish)
            roles.add("fish");
    ...
    function catchFish(command) {
        const party = resolveParty();
        if (!party || typeof party.getFish !== "function")
            throw new Error("party getFish is unavailable");
        ...
        if (party) {
            ...
            while (proto && depth < 4) {
                safeOwnPropertyNames(proto).forEach((key) => {
                    if (matchFishing(key) && typeof proto[key] === "function" && !methods.includes(key))
                        methods.push(key);
                });
                proto = Object.getPrototypeOf(proto);
                depth += 1;
    ...
    function execute(command) {
        ...
        if (type === "commonEvent.run") {
            ...
            if (!temp || typeof temp.reserveCommonEvent !== "function")
                throw new Error("reserveCommonEvent is unavailable");
        ...
        if (type === "gold.add") {
            ...
            if (typeof party.gainGold === "function")
                withRatesSuppressed(() => party.gainGold(amount));
            else
                party._gold = Math.max(0, Number(party._gold || 0) + amount);
        ...
        if (type === "gold.set") {
            ...
            if (typeof party.gainGold === "function")
                withRatesSuppressed(() => party.gainGold(value - current));
            else
                party._gold = value;
            return { gold: safeGold(party) };
        }
        ...
        if (type === "item.add") {
            ...
            const data = resolveData(kind);
            if (!data)
                throw new Error(`${kind} data is unavailable`);
            const item = data[Math.floor(requireNumber(command.id, "id"))];
            if (!item)
                throw new Error(`${kind} ${command.id} not found`);
        ...
        if (type === "actor.add" || type === "actor.unlock") {
            const party = resolveParty();
            if (!party || typeof party.addActor !== "function")
                throw new Error("party addActor is unavailable");
            const id = Math.floor(requireNumber(command.id, "id"));
            party.addActor(id);
            refreshMapAndWindows();
        ...
        if (type === "actor.remove") {
            const party = resolveParty();
            if (!party || typeof party.removeActor !== "function")
                throw new Error("party removeActor is unavailable");
            const id = Math.floor(requireNumber(command.id, "id"));
        ...
        if (type === "actor.level.set") {
            ...
            if (typeof actor.changeLevel === "function")
        ...
        if (type === "actor.exp.add") {
            ...
            if (typeof actor.gainExp === "function")
                withRatesSuppressed(() => actor.gainExp(amount));
        ...
        if (type === "actor.vitals.set") {
            ...
            if (command.hp !== undefined && command.hp !== "") {
                ...
                if (typeof actor.setHp === "function")
                    actor.setHp(hp);
                else
                    actor._hp = hp;
            }
            if (command.mp !== undefined && command.mp !== "") {
                ...
                if (typeof actor.setMp === "function")
                    withNoCostSuppressed(() => actor.setMp(mp));
                else
                    actor._mp = mp;
                resetNoCostBaselines();
            }
            if (command.tp !== undefined && command.tp !== "") {
                ...
                if (typeof actor.setTp === "function")
                    withNoCostSuppressed(() => actor.setTp(tp));
                else
                    actor._tp = tp;
                resetNoCostBaselines();
            }
        ...
        if (type === "actor.name.set") {
            ...
            if (typeof actor.setName === "function")
                actor.setName(name);
            else
                actor._name = name;
            refreshActor(actor);
        ...
        if (type === "actor.skill.learn") {
            ...
            if (typeof actor.learnSkill !== "function")
                throw new Error("actor learnSkill is unavailable");
        ...
        if (type === "actor.skill.forget") {
            const actor = requireActor(command.id);
            const skillId = Math.floor(requireNumber(command.skillId, "skillId"));
            if (typeof actor.forgetSkill !== "function")
                throw new Error("actor forgetSkill is unavailable");
