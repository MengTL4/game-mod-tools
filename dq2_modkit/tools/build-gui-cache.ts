...
function buildHuntMapCatalog(mapInfos, troops, enemies, tables) {
  ...
  return Array.from(ids)
    ...
    .map((id) => {
      ...
      const enemyNames = Array.from(new Set<string>(troopDetails.flatMap((detail) => detail.enemyNames).filter(Boolean)));
      const dropNames = Array.from(new Set<string>(troopDetails.flatMap((detail) => detail.dropNames).filter(Boolean)));
