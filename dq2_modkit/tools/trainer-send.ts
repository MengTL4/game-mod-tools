...
function usage() {
  ...
  npx tsx trainer-send.ts trainer.options.set expRate=2 goldRate=2 dropRate=3 skillRate=5 noSkillCost=true oneHitKill=true invincible=true
  npx tsx trainer-send.ts trainer.hooks.info
  npx tsx trainer-send.ts battle.start troopId=192
  ...
  npx tsx trainer-send.ts offlineHunt.probe troopId=192 times=50
  ...
  npx tsx trainer-send.ts offlineHunt.run troopId=192 times=10 nativeDrops=true enemyBook=true save=false autoSellQualities=0,1,2,3,4,5,6,7,8
  npx tsx trainer-send.ts runtime.search fish fishing 钓 鱼
  npx tsx trainer-send.ts runtime.inspect window.TK.$ 300
  npx tsx trainer-send.ts fishing.info
  npx tsx trainer-send.ts fishing.options.set autoSuccess=true powerRate=5 powerBonus=20
  npx tsx trainer-send.ts fishing.power.set 100
  npx tsx trainer-send.ts fishing.power.add 10
  npx tsx trainer-send.ts fishing.items.add rod,bait 1
  npx tsx trainer-send.ts fishing.qualifications.unlock
  npx tsx trainer-send.ts fishing.catch 1 5
...
const command: any = makeCommand(process.argv.slice(2));
if (!command) {
  usage();
  process.exit(0);
}
