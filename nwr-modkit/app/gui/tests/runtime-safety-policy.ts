...
function assertBridgeDataDumpSafety(sourceName, source) {
  assertIncludes(sourceName, source, [
    ...
    "data.dump outputDir must be inside nwr-modkit/output or .omo/evidence",
  ...
  if (source.includes("path.join(projectRoot, \"runtime\"),")) {
    throw new RuntimeSafetyPolicyError(`${sourceName} must not allow data.dump output under nwr-modkit/runtime`);
  }
