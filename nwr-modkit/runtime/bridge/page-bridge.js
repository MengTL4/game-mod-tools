...
(function () {
    ...
    const projectRoot = bridgeConfig.projectRoot || process.env.DQ2_MODKIT_ROOT || path.join(gameRoot, "dq2-modkit");
    ...
    function resolveDataDumpOutputDir(value) {
        ...
        if (!dataDumpOutputRoots.some((root) => isInsidePath(outputDir, root))) {
            throw new Error("data.dump outputDir must be inside nwr-modkit/output or .omo/evidence");
        }
        return outputDir;
    }
    ...
    function maybeDumpParsedData(source, value, rawText) {
        try {
            ...
            if (!fileName || value == null || typeof value !== "object")
                return;
    ...
    function installInGameOverlay() {
        try {
            if (!document || document.getElementById("codex-trainer-overlay"))
                return false;
      ...
      `;
            ...
            function render() {
                ...
                for (const name of ["noSkillCost", "oneHitKill", "invincible"]) {
                    ...
                    if (button)
                        button.classList.toggle("active", !!bridge.options[name]);
                }
            }
    ...
    function trainerHooksWanted() {
        if (bridgeConfig.trainerHooks === false)
            return false;
        const options = bridge.options;
        return (Number(options.expRate || 1) !== 1 ||
    ...
    function prisonGuardValue(guard) {
        if (guard.kind === "gold")
            return prisonNumber(safeGold(resolveParty()));
        if (guard.kind === "variable")
            return prisonNumber(variableValue(guard.itemId));
    ...
    function prisonGuardPath(guard) {
        if (guard.kind === "gold")
            return "party._gold";
        if (guard.kind === "variable")
            return `variables._data.@a[${guard.itemId}]`;
    ...
    function actorParamPrisonCheck() {
        ...
        try {
            ...
            if (actor && typeof actor.param === "function")
                rawValue = actor.param(9);
        }
        catch (_) { }
        if (rawValue == null) {
            return prisonCheck("actor-2-param-9", "运行时提示", "无名感知运行时判定", "CommonEvent 344: actor(2).param(9)", "无法读取", "< 19996", "CE344：提示终身监禁，Switch520 ON，禁用存档，传送 Map695", "warning", false, "该值依赖游戏运行时公式、装备和插件效果；GUI 会在 bridge 可读取时确认。");
        }
        const value = prisonNumber(rawValue);
        const hit = value >= 19996;
        return prisonCheck("actor-2-param-9", "运行时判定", "无名感知运行时判定", "CommonEvent 344: actor(2).param(9)", value, "< 19996", "CE344：提示终身监禁，Switch520 ON，禁用存档，传送 Map695", hit ? "danger" : "ok", false, hit ? "运行时已命中该阈值；需要手动检查装备、状态或插件参数来源。" : "");
    }
    function collectPrisonGuardReport() {
        const checks = [];
        const punishmentSwitch = switchValue(520) === true;
        checks.push(prisonCheck("switch-520", "惩处状态", "破坏规则惩处开关", "switches._data.@a[520]", punishmentSwitch ? "true" : "false", "false", "Map008 Event 1 Page 20 会因该开关切换事件页；部分判定会先打开此开关。", punishmentSwitch ? "danger" : "ok", punishmentSwitch));
        PRISON_NUMERIC_GUARDS.forEach((guard) => {
            const value = prisonGuardValue(guard);
            const hit = value >= guard.limit;
            checks.push(prisonCheck(guard.id, guard.group, guard.label, prisonGuardPath(guard), value, `< ${guard.limit}`, guard.effect, hit ? "danger" : "ok", hit));
        });
        ...
        PRISON_MISSING_ITEM_GUARDS.forEach((guard) => {
            ...
            checks.push(prisonCheck(guard.id, guard.group, guard.label, `CE${guard.commonEventId}: party has actor ${guard.actorName} #${guard.actorId}, requires item ${guard.itemName} #${guard.itemId}`, actorInParty ? `actor in party, item=${itemCount}` : "actor not in party", "actor absent or item >= 1", guard.effect, hit ? "danger" : "ok", hit, hit ? "修复会补 1 个要求物品，不会移除角色。" : ""));
        });
    ...
    function prisonSetBagCount(kind, bagName, id, value) {
        ...
        try {
            ...
            if (party[bagName] && typeof party[bagName] === "object") {
                if (value <= 0)
                    delete party[bagName][String(id)];
                else
                    party[bagName][String(id)] = value;
                return true;
            }
        }
        catch (_) { }
    ...
    function repairPrisonGuardRisks() {
        ...
        PRISON_NUMERIC_GUARDS.forEach((guard) => {
            if (prisonGuardValue(guard) < guard.limit)
                return;
            let ok = false;
            if (guard.kind === "gold")
                ok = prisonSetGold(guard.safeValue);
            else if (guard.kind === "variable")
                ok = setVariableValue(guard.itemId, guard.safeValue) === guard.safeValue;
            else
                ok = prisonSetBagCount(guard.kind, guard.bagName, guard.itemId, guard.safeValue);
            if (ok)
                pushFixed(guard.id);
        });
        PRISON_MISSING_ITEM_GUARDS.forEach((guard) => {
            if (!prisonPartyActorIds().includes(guard.actorId) || prisonBagCount("_items", guard.itemId) > 0)
                return;
            if (prisonSetBagCount("item", "_items", guard.itemId, 1))
                pushFixed(guard.id);
        });
    ...
    function resolveIconExportOutputDir(value) {
        ...
        if (!allowedRoots.some((root) => isInsidePath(outputDir, root))) {
            throw new Error("asset.iconSet.export outputDir must be inside app/gui/icons or nwr-modkit/output");
        }
        return outputDir;
    }
    ...
    function canvasPngBuffer(canvas) {
        ...
        if (index < 0)
            throw new Error("IconSet canvas did not produce a PNG data URL");
    ...
    function writeIconSetBitmap(command, bitmap) {
        ...
        if (!source)
            throw new Error("IconSet image source is unavailable");
        ...
        if (width < 32 || height < 32)
            throw new Error("IconSet dimensions are unavailable");
        const canvas = document.createElement("canvas");
        canvas.width = width;
        canvas.height = height;
        const context = canvas.getContext("2d");
        if (!context)
            throw new Error("IconSet canvas context is unavailable");
    ...
    function scheduleIconSetExport(command, bitmap) {
        ...
        const runExport = function () {
            ...
            try {
                ...
                if (attempts >= 80)
                    finish(false, { error: "IconSet bitmap was not ready after 20 seconds" });
            }
            catch (error) {
                bridge.lastError = String(error && error.stack || error);
    ...
    function exportIconSet(command) {
        ...
        if (!bitmap)
            throw new Error("IconSet bitmap is unavailable");
        if (typeof bitmap.isError === "function" && bitmap.isError())
            throw new Error("IconSet bitmap failed to load");
        if (typeof bitmap.isReady === "function" && !bitmap.isReady())
            return scheduleIconSetExport(command, bitmap);
    ...
    function patchBridgeTickHooks() {
        const sceneManager = resolveSceneManager();
        if (!sceneManager || sceneManager.__codexBridgeTickPatched)
            return false;
        let patched = false;
        ["updateMain", "update", "renderScene"].forEach((name) => {
            if (typeof sceneManager[name] !== "function")
                return;
    ...
    if (bridgeConfig.savePathPatch !== false) {
        const patchTimer = setInterval(function () {
            if (patchSavePaths()) {
                if (!bridgeConfig.disableTitleRefresh)
