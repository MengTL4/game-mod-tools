...
function parseArgs(argv: string[]) {
  const options = { gameRoot: "", json: false, assert: false, checkTargets: [] as string[], help: false };
...
function assertHashes(report) {
  const mismatches = Object.entries(report.protectedFiles)
    .filter(([name, data]: [string, { sha256?: string }]) => data.sha256 !== BASELINE_HASHES[name])
    .map(([name]) => name);
