import type { FeatureClassification, PanelPolicy } from "./feature-audit-policy";
...
export function panelGuardText(policy: PanelPolicy): string {
  const evidence = policyEvidenceText(policy);
  return `A1 ${policy.classification}: ${policy.rationale}. ${evidence}`;
}

function policyEvidenceText(policy: PanelPolicy): string {
