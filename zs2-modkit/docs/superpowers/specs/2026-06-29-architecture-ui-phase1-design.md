...
- Runtime source and `skills/assets/zs2-modkit_template` source are duplicated, so version or bugfix changes can drift.
...
Phase 1 will not remove the template copy under `skills/assets/zs2-modkit_template`; it will add checks and repeatable sync steps so the copy does not silently drift.
...
Every source change that affects runtime behavior must be reflected in `skills/assets/zs2-modkit_template`.
