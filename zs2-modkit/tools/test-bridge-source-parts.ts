...
assert.doesNotMatch(buildScript, /const parts = \[\s*"page-bridge\.ts"\s*\]/, "build-bridge should not use the monolithic source as its only part");

const joined = parts
  .map((relativePath) => readFileSync(path.join(projectRoot, relativePath), "utf8").trimEnd())
  .join("\n") + "\n";

assert.equal(joined, readFileSync(path.join(projectRoot, "runtime", "bridge", "src", "page-bridge.ts"), "utf8"), "bridge source should be generated from parts");
