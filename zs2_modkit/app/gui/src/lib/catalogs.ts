...
import { looseNumber } from "./format";
...
export type CatalogMap = Record<string, any[]>;

export function loadCatalogs(context: CatalogContext): CatalogMap {
  const systemData = readJson(path.join(context.dataDir, "System.json")) || {};
...
export function catalogName(catalogs: CatalogMap, kind: string, id: any): string {
  const item = catalogEntry(catalogs, kind, id);
  return item ? item.name : "";
}

export function catalogEntry(catalogs: CatalogMap, kind: string, id: any): any {
...
function loadTitleCatalog(context: CatalogContext): any[] {
  ...
  try {
    ...
    fs.readdirSync(context.useDataDir)
      ...
      .forEach((name) => {
        const data = readJson(path.join(context.useDataDir, name));
...
function loadCostumeCatalog(context: CatalogContext): any[] {
  const data = readJson(context.costumeDataPath);
  const rows: any[] = [];
  const seen = new Set<number>();
  const push = (entry: any, key: any) => {
...
function buildAllItemCatalog(catalogs: CatalogMap, itemKindLabels: Record<string, string>): any[] {
...
function loadMapCatalog(context: CatalogContext): any[] {
  const data = readJson(path.join(context.dataDir, "MapInfos.json")) || [];
  if (!Array.isArray(data)) return [];
  return data
    .filter((entry) => entry && Number.isFinite(Number(entry.id)) && entry.name)
    .map((entry) => {
...
function localDropTables(context: CatalogContext): Record<string, any[]> {
  return {
    item: readJson(path.join(context.dataDir, "Items.json")) || [],
    weapon: readJson(path.join(context.dataDir, "Weapons.json")) || [],
    armor: readJson(path.join(context.dataDir, "Armors.json")) || []
  };
}

function localDropNamesOfEnemy(context: CatalogContext, enemy: any, tables = localDropTables(context)): string[] {
  const names: string[] = [];
...
function localTroopDetails(context: CatalogContext, troop: any, enemies: any[], tables = localDropTables(context)): any {
  const visibleMembers = (troop && troop.members || []).filter((member: any) => member && !member.hidden && Number(member.enemyId) > 0);
  const enemyRows = visibleMembers.map((member: any) => enemies[Number(member.enemyId)]).filter(Boolean);
...
function loadHuntMapCatalog(context: CatalogContext): any[] {
  const mapInfos = readJson(path.join(context.dataDir, "MapInfos.json")) || [];
  const troops = readJson(path.join(context.dataDir, "Troops.json")) || [];
  const enemies = readJson(path.join(context.dataDir, "Enemies.json")) || [];
  ...
  try {
    ...
    return Array.from(ids)
      .sort((a, b) => a - b)
      .map((id) => {
        const map = readJson(path.join(context.dataDir, `Map${String(id).padStart(3, "0")}.json`));
        const info = mapInfos[id] || {};
        if (!map && !info.name) return null;
        const encounters = map && Array.isArray(map.encounterList)
          ? map.encounterList.filter((entry: any) => entry && Number(entry.troopId) > 0)
            .map((entry: any) => ({
      ...
      .filter(Boolean)
      .sort((a: any, b: any) => Number(!a.hasEncounters) - Number(!b.hasEncounters) || a.id - b.id);
  } catch {
    return [];
  }
}
...
function loadCommonEventCatalog(context: CatalogContext): any[] {
  const data = readJson(path.join(context.dataDir, "CommonEvents.json")) || [];
  if (!Array.isArray(data)) return [];
  return data
    .filter((entry) => entry && Number.isFinite(Number(entry.id)) && entry.name)
    .map((entry) => {
