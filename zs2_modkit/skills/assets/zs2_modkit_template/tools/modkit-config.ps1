...
function Resolve-Zs2GameRoot {
  ...
  if ($env:ZS2_GAME_ROOT) {
    $candidates.Add([pscustomobject]@{ Value = [string]$env:ZS2_GAME_ROOT; Base = $ProjectRoot })
  }
...
function Set-Zs2RuntimeEnvironment {
  param(
    [Parameter(Mandatory = $true)]
    [string]$ProjectRoot,
    [Parameter(Mandatory = $true)]
  ...
  $env:ZS2_MODKIT_ROOT = (Resolve-Path -LiteralPath $ProjectRoot).Path
  $env:ZS2_GAME_ROOT = (Resolve-Path -LiteralPath $GameRoot).Path
}
